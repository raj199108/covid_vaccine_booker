...
KVDB_URL = 'https://kvdb.io/3YgXf9PHYHbX6NsF7zP6Us/{0}'
...
def generate_token_OTP_auto(mobile, request_header):
    """
    This function generate OTP and returns a new token
    """
    if not mobile:
    ...
    while not valid_token:
        try:
            # storage_url = KVDB_URL.format(mobile)
            # print("Clearing OTP bucket:{}".format(storage_url))
            # requests.put(storage_url, data={})
            # print("Cleared OTP bucket:{}".format(storage_url))
